...
class Save extends \Magento\User\Controller\Adminhtml\User
{
    ...
    public function execute()
    {
    	...
    	if(isset($data['delivery_area']))
        {
            ...
            //custom @sagarppanchal

            $OM = \Magento\Framework\App\ObjectManager::getInstance();
            $qtCol = $OM->create('Mobicommerce\Area\Model\Grid')->getCollection()
                        ->addFieldToFilter('entity_id', array('in' => $data['delivery_area']));

            foreach($qtCol as $item)
            {
                $item->setUsers($data['user_id']);
            }
            $qtCol->save();
            ...
            // upto here custom @sagarppanchal
        }
